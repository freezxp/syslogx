...
function Sources(){const sources=useQuery({queryKey:['sources'],queryFn:({signal})=>api.sources(signal),refetchInterval:10000});return <div className="page"><PageHeader eyebrow="INGEST" title="Sources" description="Configured listeners and their current process state."/><section className="panel source-table"><div className="source-head"><span>Name</span><span>Protocol</span><span>Container address</span><span>Status</span></div>{sources.data?.data.map(s=><div className="source-row" key={s.id}><span><div className="source-icon"><Server size={16}/></div><strong>{s.name}</strong></span><code>{s.protocol.toUpperCase()}</code><code>{s.address}</code><em><i/>{s.status}</em></div>)}</section>{sources.error&&<Notice>Could not load configured sources.</Notice>}<Notice>Source changes currently require editing <code>config/syslogx.yaml</code> and restarting Syslogx. Runtime source management is not enabled.</Notice></div>}
